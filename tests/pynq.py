...
class HardwareLR:
    ...
    def _run_hw_batch(self, x_float, y_float):
        ...
        print(f"time to load: {t2- t1}, time to write reg addresses: {t3-t2}, time to read hw result: {t5-t4}")
        self.ata += hw_ata
        self.atb += hw_atb
    ...
    def stream_chunk(self, samples_norm):
        ...
        self.weights, _, _, _ = np.linalg.lstsq(self.ata, self.atb, rcond=None)
        t4 = time.time()
        
        print(f"cflatten sample batch: {t2- t1}, solve weights: {t4-t3}")
    def reset(self):
        self.weights = np.zeros((self.D, 1))
        self.ata = np.zeros((self.num_params, self.num_params), dtype=np.float64)
        self.atb = np.zeros((self.num_params, 1), dtype=np.float64)
...
class LinearRegressionEngine:
    ...
    def reset(self):
        self.unoptimised_sw_lr.reset()
        self.optimised_sw_lr.reset()
...
class Engine:
    def __init__(self, ip, binance_ws_client):
...
if __name__ == "__main__":
    ...
    binance_ws_client.update_url(target)
    try:
        while True:
            ...
            with eng.data_lock:
                ...
                if num_labels >= 15:
                    ...
                    response = requests.get(f"{API_BASE_URL}/instruction").json()
                    potential_target = response["asset"]
                    if potential_target != target:
                        target = potential_target
                        binance_ws_client.update_url(target)
                        print(f"[SWITCH] new target: {target}\n")
                        eng.reset()
            ...
            if data_copy is not None:
                ...
                eng.lr_engine.post_weights(target)  # ← upload to API
