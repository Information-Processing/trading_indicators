...
class BinanceWSClient:
    ...
    def update_url(self, target):
        symbol = target.lower()
        self.url = f"wss://stream.binance.com:9443/stream?streams={symbol}usdt@trade/{symbol}usdt@depth20@100ms"
        print(f"[WS] switching to {symbol.upper()}: {self.url}")
        # Clear stale data from the previous asset before reconnecting.
        with self.lock:
            self.trades.clear()
            self.order_book = {"bids": [], "asks": []}
            self.last_price = 0.0
