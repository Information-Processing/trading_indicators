...
module pdm_ser #(
    ...
    parameter real C_PDM_FREQ_MHZ = 3.0
)(
    input wire clk_i,
    input wire rst_i,
    input wire en_i,
    ...
    localparam integer CLK_LIMIT = (C_SYS_CLK_FREQ_MHZ / (C_PDM_FREQ_MHZ * 2.0)) - 1;
    ...
    assign pwm_audio_o = pdm_s_tmp[C_NR_OF_BITS-1];
    ...
    always @(posedge clk_i) begin
        if (rst_i) begin
            ...
            clk_int <= 1'b0;
            clk_intt <= 1'b0;
        end else if (cnt_clk == CLK_LIMIT) begin 
            cnt_clk <= 0;
            clk_int <= ~clk_int;
            clk_intt <= clk_int;
        end else begin
