...
def _find_libaudio():
    """Use libaudio.so from same dir as this file, or from PYNQ system install."""
    local = os.path.join(LIB_SEARCH_PATH, "libaudio.so")
    if os.path.isfile(local):
        return local
    try:
        import pynq
        pynq_dir = os.path.dirname(os.path.realpath(pynq.__file__))
        for rel in ["lib/libaudio.so", "lib/_pynq/_audio/libaudio.so"]:
            path = os.path.join(pynq_dir, rel)
...
class AudioDirect(DefaultIP):
    ...
    def __init__(self, description, gpio_name=None): # not changed
        ...
        self._ffi = cffi.FFI()
        self._libaudio = self._ffi.dlopen(_find_libaudio())
    ...
    bindto = ['xilinx.com:user:audio_direct:1.1', 'xilinx.com:user:audio_direct_v1_1:1.0']
